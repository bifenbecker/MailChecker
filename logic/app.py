import logic.emaillib
from logic.emaillib.imap import IMAP

from logic.database.models import Account, Message
...
@orm.db_session
def validation(db: SQLiteDB, protocol: IMAP, account: dict, save_path: str, dir_name: str,rm_dir: bool) -> tuple:
    ...
    except logic.emaillib.imap.Error:
...
def log_query(save_path: str, query: str, account: dict, amount: int, rm_log: bool):
...
@orm.db_session
def _main(db: SQLiteDB, accounts_list: list, save_path: str, dir_name: str,rm_dir:bool,rm_log:bool, search_params: dict = None):
    ...
    for account in accounts_list:
        ...
        conn, acc = validation(db, IMAP, account, save_path, dir_name,rm_dir)
        ...
        if conn and acc:
            ...
            log_query(os.path.join(save_path, dir_name), search_params["_filter"], account, amount, rm_log)


def run(user,path,fname,sqlite_db):


    rm_dir = True
    rm_log = True
    file_path = os.path.join(path,'mails.txt')
    file_name = fname
    save_path = ""
    ...
    _main(sqlite_db, [user], path, file_name,rm_dir,rm_log, search_params)
